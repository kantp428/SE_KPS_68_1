...
export default function PatientAppointmentListPage() {
  ...
  const { updateAppointmentStatus, loading: updateLoading } =
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "COMPLETED":
        return (
          <Badge className="bg-sky-100 text-sky-700 hover:bg-sky-200 border-sky-200">
            เสร็จสิ้น
          </Badge>
        );
      default:
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-5xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-sky-950 dark:text-foreground">
            รายการการจองของฉัน
          </h1>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-card rounded-3xl border border-dashed border-sky-200 dark:border-border gap-6 shadow-sm">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-xl font-semibold text-sky-900 dark:text-foreground">
              ไม่พบรายการจองที่ยืนยันแล้ว
            </h3>
          ...
          {appointments
            ...
            .map((appointment) => {
              ...
              return (
                <Card
                  key={appointment.id}
                  className="overflow-hidden border-sky-100 dark:border-border shadow-sm transition-all hover:shadow-md hover:border-sky-200 dark:hover:border-primary/50 rounded-2xl"
                >
                  ...
                  <CardFooter className="pt-2 pb-6 px-6">
                    {isCancellable ? (
                        ...
                        <AlertDialogContent className="rounded-2xl">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              เมื่อยกเลิกแล้วคุณจะไม่สามารถกู้คืนรายการจองนี้ได้
                              แต่สามารถจองใหม่ได้ภายหลัง
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                      ...
                      <Button
                        variant="ghost"
                        className="w-full rounded-xl cursor-default opacity-50"
                        disabled
                      >
