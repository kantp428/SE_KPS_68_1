...
function layoutDay(events: WorkSchedule[]): LayoutItem[] {
  ...
  const sorted = [...events].sort(
    (a, b) => parseTimeH(a.starttime) - parseTimeH(b.starttime),
  );
  ...
  for (const ev of sorted) {
    ...
    for (const lane of lanes) {
      if (
        parseTimeH(ev.starttime) >= parseTimeH(lane[lane.length - 1].endtime)
      ) {
        lane.push(ev);
        placed = true;
        break;
  ...
  for (let li = 0; li < lanes.length; li++) {
    for (const ev of lanes[li]) {
      const evS = parseTimeH(ev.starttime),
        evE = parseTimeH(ev.endtime);
      const concurrent = lanes.filter((lane) =>
        lane.some(
          (x) => parseTimeH(x.starttime) < evE && parseTimeH(x.endtime) > evS,
        ),
      ).length;
...
function DetailModal({
  ...
  const dur = Math.round(
    (parseTimeH(ev.endtime) - parseTimeH(ev.starttime)) * 60,
  );
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`h-1.5 w-full rounded-t-xl ${ev.is_active ? "bg-blue-100 border-b border-blue-200" : "bg-red-50 border-b border-red-200"}`}
        />
        ...
        <div className="px-5 pb-5 space-y-4">
          <div
            className={`rounded-lg px-4 py-3 ${ev.is_active ? "bg-blue-50 border border-blue-200" : "bg-red-50 border border-red-200"}`}
          >
            <p
              className={`text-base font-bold ${ev.is_active ? "text-blue-800" : "text-red-700"}`}
            >
              {fmtFullDateTH(parseDate(ev.date))}
            </p>
            <div className="flex items-center gap-1.5 mt-1.5">
              <Clock
                className={`w-3.5 h-3.5 ${ev.is_active ? "text-blue-500" : "text-red-400"}`}
              />
              <p
                className={`text-sm font-semibold ${ev.is_active ? "text-blue-700" : "text-red-600"}`}
              >
                {fmtTime(ev.starttime)} – {fmtTime(ev.endtime)}
              </p>
              <span
                className={`text-xs ${ev.is_active ? "text-blue-500" : "text-red-400"}`}
              >
                · {durLabel}
              </span>
            </div>
          ...
          >
            <span
              className={`w-2 h-2 rounded-full ${ev.is_active ? "bg-green-400" : "bg-red-400"}`}
            />
...
export default function StaffWeeklyCalendar({
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.log("PATH", `/api/work-schedule?${p}`);
        const json = await res.json();
        const data: WorkSchedule[] = json.data ?? [];
        setSchedules(data);
        if (!nameProp && data.length > 0 && data[0].staff) {
          setStaffName(
            `${data[0].staff.first_name} ${data[0].staff.last_name}`,
          );
        }
      } catch (e) {
        console.error(e);
  ...
  const weekSchedules = schedules.filter((s) =>
    weekDays.some((d) => isSameDay(parseDate(s.date), d)),
  );
  const activeCount = weekSchedules.filter((s) => s.is_active).length;
  const inactiveCount = weekSchedules.filter((s) => !s.is_active).length;
  const totalMins = weekSchedules
    .filter((s) => s.is_active)
    .reduce(
      (acc, s) =>
        acc +
        Math.round((parseTimeH(s.endtime) - parseTimeH(s.starttime)) * 60),
      0,
    );
  ...
  const dayLayouts = weekDays.map((day) =>
    layoutDay(schedules.filter((s) => isSameDay(parseDate(s.date), day))),
  );
  const daySchedules = weekDays.map((day) =>
    schedules.filter((s) => isSameDay(parseDate(s.date), day)),
  );
  const scheduleDates = new Set(
    schedules.filter((s) => s.is_active).map((s) => s.date),
  );
  ...
  const roleLabel =
    firstStaff?.staff_role === "DOCTOR"
      ? "แพทย์"
      : firstStaff?.staff_role === "MED_ASSISTANT"
        ? "ผู้ช่วยแพทย์"
        : "";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        {mode === "staff" && (
          <Card className="border border-border px-4 py-3 col-span-2 sm:col-span-1">
            ...
            <p
              className={`text-2xl font-bold mt-1 ${inactiveCount > 0 ? "text-red-500" : "text-foreground"}`}
            >
              {inactiveCount}
              <span className="text-sm font-normal text-muted-foreground ml-1">
                วัน
      ...
      <Card className="rounded-md border border-border bg-card overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/30">
          ...
          <span className="text-sm font-semibold text-foreground">
            {fmtMonthYear(weekStart, weekEnd)}
          </span>
        ...
        <div className={loading ? "opacity-40 pointer-events-none" : ""}>
          <div>
            ...
            {weekDays.map((day, di) => {
              ...
              const maxLanes = layout.length
                ? Math.max(...layout.map((e) => e.totalLanes))
                : 1;
              ...
              return (
                ...
                >
                  ...
                  {/* hour cells */}
                  {HOURS.map((hour, hi) => {
                    const startHere = layout.filter(
                      (ev) =>
                        parseTimeH(ev.starttime) >= hour &&
                        parseTimeH(ev.starttime) < hour + 1,
                    );
                    return (
                      <div
                        key={hi}
      ...
      {mode === "staff" && selected && (
        <DetailModal ev={selected} onClose={() => setSelected(null)} />
      )}
    </div>
  );
}
